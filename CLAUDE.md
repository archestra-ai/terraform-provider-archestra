...
- Backend models all four agent variants (`agent`, `llm_proxy`, `mcp_gateway`, plus the legacy `profile` type) on a single `agents` table with an `agentType` discriminator. The provider exposes `archestra_agent` and `archestra_mcp_gateway`; neither the legacy `profile` type nor `llm_proxy` is surfaced. The LLM proxy became a platform singleton that needs no configuration — there is nothing to declare, and `GET /api/agents/:id` refuses to read one — so `archestra_llm_proxy` was removed in v1.6.0.
