...
- The `tool_id` attribute on `archestra_tool_invocation_policy` and `archestra_trusted_data_policy` is the **bare tool UUID** (matching the `tools` table — the backend stores `toolId` directly). It is NOT the agent-tool assignment composite ID. Preferred lookup: `archestra_mcp_server_installation.<n>.tool_id_by_name["<server>__<short>"]` — one line, no extra data source. Fallback: `archestra_agent_tool.<n>.tool_id` (when the assignment is also managed by Terraform) or `data.archestra_mcp_server_tool.<n>.id` (for installs not managed by Terraform).
- The Guardrails UI's per-row policy controls map to **two separate provider resources per axis**, not one with a mode flag:
  - The **`DEFAULT` row dropdown** (Tool Call Policies / Tool Result Policies) maps to `archestra_tool_invocation_policy_default` / `archestra_trusted_data_policy_default`. These hit `POST /api/tool-invocation/bulk-default` — upsert by tool_id, accept a one-element `tool_ids` set for single-tool defaults.
  - The **`Add Policy` button** (conditional rules with `key` / `operator` / `value` matchers) maps to `archestra_tool_invocation_policy` / `archestra_trusted_data_policy`. These require non-empty `conditions`. They layer on top of the default — conditional rules evaluate first, default fires when none match.
  - Don't try to fake a default by creating a conditional policy with a wildcard regex — use the `_default` resource. Don't try to fold the two into one resource via optional `conditions` — they're distinct backend endpoints with different semantics.
- Same conditional/bulk split appears on the assignment surface: `archestra_agent_tool` (one tool to one agent) vs `archestra_agent_tool_batch` (every tool from one install to one agent in O(1) API calls). Bulk is the right default when you want "give this agent all of install X's tools."
