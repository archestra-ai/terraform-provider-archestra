...
// TestAccVirtualApiKeyResource_RecoversFromBackendDelete pins the
// Read-404 recovery contract: backend delete out-of-band → next refresh
// drops from state → plan recreates.
func TestAccVirtualApiKeyResource_RecoversFromBackendDelete(t *testing.T) {
	var capturedParentID, capturedID string
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVirtualApiKeyResourceConfig("Recovers After OOB Delete", "org"),
				Check: func(s *terraform.State) error {
					rs, ok := s.RootModule().Resources["archestra_virtual_api_key.test"]
					if !ok {
						return fmt.Errorf("archestra_virtual_api_key.test not in state")
					}
					capturedParentID = rs.Primary.Attributes["llm_provider_api_key_id"]
					capturedID = rs.Primary.Attributes["id"]
					if capturedParentID == "" || capturedID == "" {
						return fmt.Errorf("captured ids empty (parent=%q id=%q)", capturedParentID, capturedID)
					}
					return nil
				},
			},
			{
				PreConfig: func() {
					if capturedParentID == "" || capturedID == "" {
						t.Fatal("OOB delete: captured ids empty (step 1 didn't run?)")
					}
					...
					parentID, err := uuid.Parse(capturedParentID)
					if err != nil {
						t.Fatalf("OOB delete: parse parent id: %s", err)
					}
					id, err := uuid.Parse(capturedID)
					if err != nil {
						t.Fatalf("OOB delete: parse id: %s", err)
					}
					delResp, err := c.DeleteVirtualApiKeyWithResponse(t.Context(), parentID, id)
					if err != nil {
				...
				Check: func(s *terraform.State) error {
					rs, ok := s.RootModule().Resources["archestra_virtual_api_key.test"]
					if !ok {
						return fmt.Errorf("archestra_virtual_api_key.test not in state after recovery apply")
					}
...
// testAccVirtualApiKeyResourceConfig builds an org-scoped virtual key
// against a vault-backed parent. Team-scoped variants live in their own
// helpers because the team resource adds a dependency edge.
func testAccVirtualApiKeyResourceConfig(name, _ string) string {
	return fmt.Sprintf(`
resource "archestra_llm_provider_api_key" "parent" {
