...
func (d *UserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	userID, err := uuid.Parse(data.ID.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(
			"Invalid User ID",
	...
	// Get user data
	userResp, err := d.client.GetUserWithResponse(ctx, userID)
	if err != nil {
	...
	if userResp.JSON200 == nil {
		resp.Diagnostics.AddError("Unexpected API Response", fmt.Sprintf("Expected 200 OK, got status %d", userResp.HTTPResponse.StatusCode))
		return
	}
	...
	// data.EmailVerified = types.BoolValue(user.EmailVerified) // User struct matches what I defined in manual_extensions.go which doesn't have EmailVerified yet
