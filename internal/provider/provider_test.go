...
func TestProviderResources_RegistrationCount(t *testing.T) {
	...
	// We expect 13 resources to be registered
	expectedCount := 13 // agent, mcp_server, mcp_server_installation, team, team_external_group, tool_invocation_policy, trusted_data_policy, token_price, limit, optimization_rule, user, role, user_role_assignment
...
func TestProviderDataSources_RegistrationCount(t *testing.T) {
	...
	// We expect 7 data sources to be registered
	expectedCount := 7 // team, team_external_groups, agent_tool, mcp_server_tool, token_prices, user, role
