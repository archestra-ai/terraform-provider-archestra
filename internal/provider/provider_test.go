...
func testAccRequireEnterprise(t *testing.T) {
	if os.Getenv("ARCHESTRA_ENTERPRISE_LICENSE_ACTIVATED") != "true" {
		t.Fatal("requires ARCHESTRA_ENTERPRISE_LICENSE_ACTIVATED=true — Enterprise Edition routes are only exposed when the backend is started with the EE license active.")
	}
}
