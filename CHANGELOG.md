...
* **Agent Runtime naming replaces background execution naming.** The `archestra_execution_credential` resource is now `archestra_runtime_credential`, and the `archestra_agent.background_execution` nested block is now `runtime`. No deprecated resource or block alias is retained. Rename the HCL block and references, then move each credential resource in state with `terraform state mv archestra_execution_credential.<name> archestra_runtime_credential.<name>`.
